...
// 首先要得到所有的数据总和
$_search_article = $_GET['search'];
$_num = $mysqli->query("SELECT * FROM article WHERE title LIKE '%$_search_article%' ORDER BY pubTime DESC")->num_rows;
$_count_page = ceil($_num / $_pagesize);
if ($_count_page == 1) {
    $_count_page = 0;
}
// 显示分页内容内容
$sql = "SELECT * FROM article WHERE title LIKE '%$_search_article%' ORDER BY pubTime DESC LIMIT $_pagenum,$_pagesize";
...
if ($mysqli_result && $mysqli_result->num_rows) {
    while ($rows = $mysqli_result->fetch_assoc()) {
        ...
        window.onload = function() {
            ...
            classVal = classVal.concat(" height");
                ...
                <div class="page-num">
                    <ul>
                        <?php for ($i = 0; $i < $_count_page; $i++) {
                            if ($_page == $i+1) {?>
                            <li><a href="article.search.php?search=<?php echo $_search_article; ?>&page=<?php echo $i+1; ?>" class="selected"><?php echo $i+1; ?></a></li>
                            <?php } else { ?>
                            <li><a href="article.search.php?search=<?php echo $_search_article; ?>&page=<?php echo $i+1; ?>"><?php echo $i+1; ?></a></li>
                        <?php }} ?>
                    </ul>
                </div>
            </div>
                ...
                <ul>
                    ...
                    if (is_array(@$_new_article)) {
                        for ($i = 0; $i < (6<count($_new_article,0)?6:count($_new_article,0)); $i++) { ?>
                            <a href="article.php?search=<?php echo $_search_article; ?>&id=<?php echo $_new_article[$i]['id']; ?>"><li><?php echo $_new_article[$i]['title']; ?></li></a>
                    <?php }} ?>
                </ul>
            </div>
        </div>
